...
import React, { useState, useEffect, FC, forwardRef, Ref, useRef } from 'react';
...
const RichTextEdit = () => {
	...
	const previewViewRef = useRef<RefParam>(null);
	// 预览
	const onPreviewView = () => {
		setContent(editor?.getHtml() as string);

		previewViewRef.current?.onOpen();
	};

	const previewCodeRef = useRef<RefParam>(null);
	// 预览
	const onPreviewCode = () => {
		setContent(editor?.getHtml() as string);

		previewCodeRef.current?.onOpen();
	};
	return (
		<div>
			<Icard>
				...
				<div className="mt-2">
					...
					<Button type="primary" onClick={onPreviewView}>
						预览效果
					</Button>
					<Button type="primary" onClick={onPreviewCode}>
						预览代码
					</Button>
				</div>
