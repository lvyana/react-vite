...
const TabsMain = () => {
	...
	const [panes, setPanes] = useState<PanesParams[]>([]);
	...
	return (
		<div className={style['tabs'] + ' mb-2'}>
			<Tabs
				...
				type="editable-card"
				onEdit={onEdit}
				items={panes.map((item) => getTabs(t(item.title || ''), item.key, item.disabled, item.closable))}></Tabs>
		</div>
	);
};
TabsMain.displayName = 'TabsMain';
