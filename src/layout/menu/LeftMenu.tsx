...
const LeftMenu = () => {
	...
	const menu = useMemo(() => getMenu(menuList, t), [menuList, t]);
 ...
 * @returns menu数据
 */
const getMenu = (menuArr: Router[], t: TFunction<'translation', undefined, 'translation'>): MenuItem[] => {
	return menuArr.reduce((acc: MenuItem[], item) => {
		if (item.show === false) return [...acc];

		if (item.children && item.children.length > 0) {
			let newItem = { ...getItem(t(item.title), item.path, item.icon), children: getMenu(item.children, t) } as MenuItem;
			return [...acc, newItem];
		} else {
			let newItem = getItem(<Link to={item.path}> {t(item.title)}</Link>, item.path, item.icon);
			return [...acc, newItem];
		}
	}, []);
};
